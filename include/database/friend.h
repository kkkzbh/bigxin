...
namespace database
{
    ...
    auto is_friend(i64 user_id, i64 peer_id) -> boost::asio::awaitable<bool>;

    /// \brief 加载某个用户的好友列表。
    /// \param user_id 当前用户 ID。
    /// \return 好友信息列表。
    auto load_user_friends(i64 user_id)
        -> boost::asio::awaitable<std::vector<FriendInfo>>;
    ...
    auto search_friend_by_account(i64 current_user_id, std::string const& account)
        -> boost::asio::awaitable<SearchFriendResult>;
    ...
    ) -> boost::asio::awaitable<FriendRequestResult>;

    /// \brief 加载"别人加我"的好友申请列表。
    /// \param user_id 当前用户 ID。
    /// \return 好友申请列表。
    auto load_incoming_friend_requests(i64 user_id)
        -> boost::asio::awaitable<std::vector<FriendRequestInfo>>;
